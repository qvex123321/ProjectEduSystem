...
//		String requestURI = (String) session.getAttribute("requestURI");
		...
		MembersBean msb = null;
		try {
			// 呼叫 loginService物件的 checkIDPassword()，傳入userid與password兩個參數
			msb = memberService.checkIdPassword(username, password);
			if (msb != null) {
				// OK, 登入成功, 將mb物件放入Session範圍內，識別字串為"LoginOK"
				session.setAttribute("LoginOK", msb);
			} else {
...
//			response.sendRedirect(response.encodeRedirectURL("administrator/admin.jsp"));
			if (msb.getPrivilegeId() == 3) {
				response.sendRedirect(response.encodeRedirectURL("member/administrator/admin.jsp"));
				return;
			}else if(msb.getPrivilegeId()==2) {
				response.sendRedirect(response.encodeRedirectURL("member/teacher/teacher.jsp"));
				return;
			}else if(msb.getPrivilegeId()==1) {
				StudentBean mb = (StudentBean)msb;
				session.setAttribute("LoginO2K", mb);
				System.out.println(mb.getSeatNo());
				response.sendRedirect(response.encodeRedirectURL("member/student/student.jsp"));
				return;
			}
...
//			RequestDispatcher rd = request.getRequestDispatcher("member/login.jsp");
			...
			response.getWriter().println("<script>alert('該會員不存在或是密碼錯誤！');location.href='member/login.jsp';</script>");
